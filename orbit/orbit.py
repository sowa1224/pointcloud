...
# 直線の方向ベクトルを計算
direction = Top_point[0,:2] - Bot_point[0,:2]
print(direction)
...
# 各点と直線との距離を計算
distances = np.abs(np.cross(points[:, :2] - Top_point[0,:2], norm_direction))
...
selected_z = selected_points[:, 2]

#から配列
Orbit_1 =np.column_stack((selected_x, selected_y, selected_z))
print(Orbit_1,Orbit_1.shape)
"""""
...
while Bot_x >= np.min(x):
    ...
    # 直線に近い点を抽出
    # 抽出した点をからの配列に追加
    extracted_points = np.append(extracted_points, selected_points2, axis=0)
